...
async def _startup() -> None:
    ...
    try:
        await bus.connect()
        asyncio.create_task(bus.subscribe("events.home", _on_event, group="household_ops:events"))
    except Exception as exc:
        logger.warning("household_ops_bus_connect_failed", error=str(exc))


async def _on_event(payload: dict) -> None:
    logger.info("household_ops_event", event_type=payload.get("type", "unknown"))
